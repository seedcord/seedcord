...
**Zero Technical Debt.** No workarounds, hacks, or temporary compatibility layers. Choose the cleanest architecture and break things if needed to get it right. After making changes that affect the framework's public surface, regenerate docs (`pnpm docs:extract`, or `pnpm docs:smoke` to extract + run the docs-engine smoke pass).
...
**Respect `Note for Agent:` comments.** The user may add these mid-flight as deliberately-failing type errors or lint errors so they surface when you run checks. Read and honor them before continuing, then remove the comment when done.
...
- **Use path aliases** (e.g. `@lib/...`, `@components/...`, `@seedcord/...`) over deep relative imports, updating `tsconfig.json` if needed. Apps under `apps/<name>` define their own alias map in their `tsconfig.json`.
- **Add deps with `pnpm add`** so the lock file updates. Inspect type declarations before relying on them.
- **Check upstream peer-dep ranges** before citing compatibility blockers. Run `curl -s https://registry.npmjs.org/<pkg>/latest`, look at the actual peer range, and check whether a newer version of the conflicting package widens it before pinning or skipping a bump.
...
- **Use package `scripts`** for common tasks, and document any new script you add. Check the closest `package.json` first and don't assume scripts exist.
...
- **`array.includes()` in a loop** → `new Set()` for O(1) membership, built once outside the loop.
- **`font-bold` on `<h1>` through `<h6>`** → `font-semibold`. Bold weight crushes counter shapes at display sizes.
...
- **No hydration mismatch sources** (`new Date()`, `Math.random()`, `window.*`) reachable from JSX during SSR. Move them into `useEffect` and back with state.
...
- **YAGNI on deps.** Never `pnpm add` a package until the code using it is written in the same commit. Unused deps are dead code, remove them and don't leave them as "future prereqs."
...
- **`./internal` is not public API.** `core`, `errors`, `types`, and `utils` each map a `./internal` export for framework-internal wiring. Public API means what the root entry exposes. A symbol reachable only through `./internal` has no consumer to document, so it gets no TSDoc block and no `@internal` tag on its members. The `exports` map in `package.json` decides this.
...
- **Tests live in `<package>/tests/`** mirroring `src/`, never `src/**/*.test.ts`. The lint/tc scripts in each package's `package.json` already cover `tests/**`.
...
- **Tests may use pragmatic fixture casts** (`as unknown as Test`) with a short comment naming what makes the cast safe. That comment covers the cast and nothing else. A stub whose shape the literal already shows gets no comment.
- **No `as any`** even in tests. ESLint rewrites `any` to `unknown`, and the real type errors surface if the cast was not justified.
