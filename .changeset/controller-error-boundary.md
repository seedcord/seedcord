...
- A `Denial` renders through `ReplySender`, a reporting `Denial` and a raw error publish to `handledException`/`unknownException`, and a `Halt` stops silently. Events are report-only and never auto-reply.
- Removes `@Catchable`, `@EventCatchable`, and the `setBreak`/`setErrored`/`shouldBreak`/`hasErrors` handler flags. Throw a `Halt` to stop a handler without a reply.
