...
Handlers reply through members on the base, `this.reply` / `defer` / `followUp` / `edit` / `delete` / `send`, with `update` / `deferUpdate` on component kinds and `showModal` on non-modal kinds (a modal handler rejects it at compile time). `this.delete()` removes the initial reply or a message the interaction sent. Autocomplete handlers get `this.respond(choices)`. Unmatched interactions dispatch to unhandled defaults that reply "Feature not implemented yet." as a ComponentsV2 text display (empty choices on autocomplete).
