...
Both fire outside any interaction, so no user sees a reply. You can [configure](/throwing/configuring) `errors.catchProcessErrors`, which turns seedcord's pair off when your application registers its own.
