---
title: Plugins
description: Add your own services to a bot with plugins, which seedcord starts and stops with it. Covers attaching a plugin, reading it from a handler, and where a plugin can run.
---
...
Every mongoose log line prints on the `db` channel, since a plugin logs on the channel named after its attach key. That lets you filter or silence one plugin's logs by the key you gave it. seedcord already logs on reserved channels of its own, so those names can't be keys.
