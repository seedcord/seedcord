...
`services` is a plain object on `core`. Each plugin takes the key after the dot, so a handler reads one as `this.core.services.users`. seedcord still starts both in attach order, stops them during shutdown, and reloads them while you develop.
...
```txt output
'services' holds a group of plugins. Attach this one under a name of its own.
```
...
- Nesting under a key that already holds a plugin throws `CorePluginGroupTaken`.
