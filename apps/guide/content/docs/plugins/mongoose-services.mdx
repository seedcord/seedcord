...
- **`@RegisterMongooseService('users')`** gives the class its key. Your handlers reach the service under that key.
...
With `modelName: 'app_user'`, mongoose reads the `app_users` collection. Your handlers still reach the class as `services.users`.
