...
If the connection fails, `init()` throws `PluginMongooseConnectionFailed`. Its message names the database from your `name` option, and mongoose's own error is on its `cause`.
