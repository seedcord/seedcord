---
title: Mongoose
description: Connect a gateway or http bot to MongoDB with the mongoose plugin, which opens and closes the connection with your bot.
---
...
If the connection fails, `init()` throws `PluginMongooseConnectionFailed`. Its message includes the database from your `name` option, and mongoose's own error is in its `cause`.
