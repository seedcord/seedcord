...
[`startup.addTask`](ref:core/CoordinatedStartup.addTask) and [`shutdown.addTask`](ref:core/CoordinatedShutdown.addTask) put that work in a phase, and seedcord runs the phases in order. A small [plugin](/plugins/) is usually the better home for it, since a plugin starts and stops at those same points, takes its own options, and logs on its own channel.
...
export default seedcord.attach('db', KyselyPostgres, {
