...
description: this.core, the object every handler reads for your config and plugins, and how to run work while your bot starts and stops.
---
...
Say your bot opens a cache of its own. Something has to fill it before the first command runs, then close it again when the bot stops. Doing that yourself means racing your own startup on one end and wiring up `SIGTERM` on the other.
...
`createSeedcord` builds a core without these phases, since its build never runs startup or shutdown at all. Both `addTask` calls still compile on an edge bot, and both throw `CoreLifecycleUnavailable` when they run. Do that work inside your handler.
