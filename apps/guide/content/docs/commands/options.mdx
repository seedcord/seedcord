---
title: Options
description: Declare options on a slash command and read them back through getters typed from what you declared, with required ones never null.
---

You marked the option required when you registered it, and `getString('query')` still handed back `string | null`. Every read opened with a check whose answer you already knew, or a `!` you hoped was right. A misspelled name compiled fine, then handed null to whoever ran the command.

Both of those come from the same gap, where nothing connects your declaration to the getter that reads it. Every option you declare becomes a getter on [`this.options`](ref:gateway/SlashHandler.options), typed from what you declared.
...
Each getter only accepts options this command declared for its own kind. A typo is a compile error.
