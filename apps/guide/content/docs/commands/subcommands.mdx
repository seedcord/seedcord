...
description: Once you attach a subcommand, the parent stops being a route. Each branch routes on its own.
---
...
Discord shows this as `/role add`. seedcord flattens it to `role/add`. You write that string on both the decorator and the generic, and a mismatch is a compile error.
...
A path runs to three segments at most, because Discord only accepts a group inside a command or subcommands inside a group.
