...
When you pass a scope yourself, seedcord skips the list. `'global'` keeps the command global. `'guild'` with ids sends it to those servers. To reach the configured servers plus more of your own, pass `'config'` among the ids.

```ts
@RegisterCommand('guild', ['config', '613425648685547541'])
```

`'config'` there stands for every id under `commands.guilds`, so that command reaches those servers and `613425648685547541`. If `commands.guilds` is empty, the same line throws `CoreCommandGuildsEmpty` at startup, and the message points to your command class.
