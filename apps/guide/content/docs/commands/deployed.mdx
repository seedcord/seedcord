...
description: Deleting a command file removes it from Discord, since your bot sends the whole list again every time it starts. seedcord commands deletes the guild commands left behind.
---
...
Discord replaces the whole scope with what arrives. Delete a command file and that command is gone the next time your bot starts. Rename one and the old name disappears with it.

## Deleting your last global command leaves it live

Your bot skips the request when a scope has nothing left in it.

That leaves one gap. Remove your **last** global command and your bot sends nothing, so the copy that Discord already has stays. The same happens to the last guild command in a server.
...
With no flags and a real terminal, it asks which servers to look at and what to remove. Anywhere without a terminal, or with `CI` set, it prints what it would need and exits.
...
For CI, `--all-guilds` covers every server your bot is in. `--yes` is required there, since `--apply` asks you to confirm the count and throws `CliCleanApplyNeedsYes` when it has no terminal to ask on.
