...
description: Add a right-click command to a user or a message in Discord, and read the user or message someone ran it on.
---

Reporting a message through a slash command means getting its id in front of the caller somehow, then having them paste it into an option. A context menu command puts the action on the message itself, so the right-click carries the target.

Discord shows one of these when someone right-clicks. It takes a name and a kind, and that click is its only input.
