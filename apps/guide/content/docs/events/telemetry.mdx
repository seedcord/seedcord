...
description: Measure how long your bot takes to answer with the telemetry keys on the bus, for every interaction, write, and event.
---
...
To tell which handler threw, read the fault's `origin`. On an event it holds the event and the handler, like `event:messageCreate:AutoMod`. Match on `dispatchId` as well, since every dispatch of that event carries the same `origin`.
