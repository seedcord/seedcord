...
description: Write your own gate with defineGate, from what a check reads and narrowing it to certain handlers to async checks and what to throw.
---

Some checks belong to your bot alone, like keeping a command to one channel or requiring a linked account. The shipped gates cover owners, roles, permissions, and cooldowns, so a check like that would otherwise go back to the top of every handler that needs it. [`defineGate`](ref:core/defineGate) turns that check into a gate you attach with the rest.
...
The `'InChannel'` string you passed to `defineGate` appears in the compile error when you attach the gate to a handler it doesn't fit. [`and`](ref:core/and) and [`or`](ref:core/or) join the names of their arms into one.
...
| `dispatch` | `DispatchContext` | the [dispatch context](/gates/dispatch-context), holding what a middleware wrote before the first gate ran |

{/* prettier-ignore-end */}

A gate that reads _only_ these fields fits any handler you attach it to. `InChannel` above reads one of them, so it works on a slash command, a button, and a message event.
...
`NoAttachments` doesn't take an argument. Export the gate itself and attach it without calling, the way [`IgnoreBots`](ref:gateway/IgnoreBots) does. `InChannel` above stays a function because it closes over a channel id.
...
On an interaction, `memberPermissions` and `appPermissions` hold the set for the channel the command ran in, after overwrites. A gate about the whole server, like one guarding a server-wide setting, needs what the roles grant instead. [`GuildPermissionsContext`](ref:core/GuildPermissionsContext) adds `memberGuildPermissions` and `appGuildPermissions`, the permission sets computed from roles alone.
...
If you throw a [`Notice`](ref:core/Notice), seedcord replies with the card its `render()` builds. If you throw a [`Silence`](ref:core/Silence), seedcord drops the request without replying, which suits an event handler. [Throwing](/replying/throwing) has both in full, plus what a check throwing anything else produces.

If you plan to pass a gate to an [`or`](ref:core/or), set `summary` in your `Notice`'s constructor, as in `this.summary = 'run it in #bots'`. When every arm refuses, the `or` [lists each arm's summary](/gates/combining#the-refusal-when-every-arm-refuses). If one arm doesn't set a summary, the `or` shows a generic refusal.
