...
[`GateContextBase`](ref:core/GateContextBase) declares the bag, so any gate that reads it fits any handler on either transport. Its [other fields](/gates/your-own) carry the caller's ids and permission bits.
