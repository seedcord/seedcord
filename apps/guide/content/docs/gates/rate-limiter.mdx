...
`Cooldown` peeks in its check and charges in its commit. Every [effect gate](/gates/effect-gates) separates its check from its commit the same way.
...
[`Store`](ref:types/Store) takes the verbs a backend supports as its generic. `'charge'` is the verb for rate limiting. A `Store<'charge'>` carries two fields plus the three [`IRateLimiter`](ref:types/IRateLimiter) methods.

The generic takes a union. If one backend serves several features, it declares `Store<'charge' | 'claim'>` and lists both verbs in `caps`. The type rejects a verb in `caps` that the generic leaves out. It doesn't check that `caps` lists every verb in the generic, so keep the two matching yourself. Rate limiting reads `'charge'` alone.
