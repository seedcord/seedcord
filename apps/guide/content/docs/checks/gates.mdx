---
title: Gates
description: Run checks before your handler with @Gated, so a caller who fails one gets a refusal and never reaches your code.
---
...
Every gate carries the context it reads, and `@Gated` checks that against the handler you put it on. `Nsfw()` reads an interaction's channel, so attaching it to the event handler below fails to compile. The error shows the gate and the handler kind.
