...
description: Pass values from a middleware to the handler, gates, and fault card of the same dispatch through the typed dispatch context.
---
...
If the value is `undefined`, `require` throws a `SeedcordError` with the code `DispatchStateMissing`, and its message includes the key you asked for. That covers a key nothing wrote, and a key written as `undefined`. A `null` value comes back as is. Usually the middleware that writes the key never registered, or its `{ kinds }` filter skipped this interaction.
