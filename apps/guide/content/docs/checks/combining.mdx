...
## and, to treat several gates as one

`@Gated(A(), B())` and `and(A(), B())` refuse under the same conditions. Use `and` when you want those gates as one value, usually to put it inside an `or`.
...
You can pass `and` or `or` as many arms as you like past two. With one arm, either is that gate on its own, so the compiler rejects it.
