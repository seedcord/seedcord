...
The reply sender reads the interaction's state once, when seedcord constructs your handler. That happens before `execute()` runs, and only seedcord's own verbs move it afterwards. If you send an ack yourself, the copy stays where the last seedcord verb left it.
