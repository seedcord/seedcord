---
title: Raw acks
description: Replying through discord.js's own methods on this.event, what it breaks, the lint rule that reports it, and the two cases that need it.
---
...
The rule resolves the receiver's type, which is how it reports `this.event.reply()` and a local alias alike. It misses a destructured `const { reply } = this.event`, since that call has no receiver left to resolve.
...
For the interaction that seedcord dispatched, pass the handler's [`sender`](ref:gateway/RepliableHandler.sender) to your helper. It's public. A helper that replies through it moves the same ack state your handler reads.
