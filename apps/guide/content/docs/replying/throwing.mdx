---
title: Throwing
description: Refuse from a handler by throwing a Notice, and what each kind of throw shows the user and reports.
---
...
Only the `or` gate reads `summary`. [Combining gates](/gates/combining) shows where it appears.
...
| `ctx.dispatch` | the [dispatch context](/gates/dispatch-context) the handler and its gates read |
