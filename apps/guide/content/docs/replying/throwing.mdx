---
title: Throwing
description: Refuse a request from a handler by throwing a Notice, so the user sees your reason on a card and your code stops there.
---
...
seedcord catches that throw at one boundary and sends the reply through the handler, so the class you throw sets what the reply says. A gate throws into the same boundary, and its refusal reaches the user the same way.
