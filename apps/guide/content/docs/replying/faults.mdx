...
Two config keys change this. `errors.defaultError` takes a class that seedcord constructs with the uuid, and `errors.ignoreApiCodes` takes a list of Discord api codes to swallow. [Configuring](/replying/error-behavior) has both in a real config.
...
Both fire outside any interaction, so no user sees a reply. You can [configure](/replying/error-behavior) `errors.catchProcessErrors`, which turns seedcord's pair off when your application registers its own.
...
Alright. In the past two pages, we've seen _how_, _what_, _where_, and _when_ to throw. But of course, we don't want to throw into the void, do we? We want to know what happened. Or rather, how to [report](/replying/reporting) it.
