...
seedcord calls `throttleKey()` before it sends each card. The base returns `null`, so each publish sends its own card. Override it to return a string to make events that share that string collapse into one card a minute.
