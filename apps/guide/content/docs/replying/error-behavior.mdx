...
description: The errors block on your bot config, from replacing the card an unknown fault shows to choosing which Discord API codes to ignore, and catching seedcord's own errors by code.
---
...
The second argument narrows by class, one of `'SeedcordError'`, `'SeedcordTypeError'`, `'SeedcordRangeError'`, or `'SeedcordAggregateError'`. The third matches one exact code. Pass `undefined` as the second argument when the class makes no difference, or pass both when you need the class and the code to match together.
