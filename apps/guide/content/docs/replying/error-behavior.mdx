...
description: Set what your bot does with an error through the errors block on your config, like the card a fault shows and what it ignores.
---

Something threw inside a handler, and whoever ran the command got seedcord's generic card. The wording on that card is yours to replace. So is the decision about which errors reach a user at all, since a message someone already deleted is nothing they can act on.

You set both on the `errors` block [`Seedcord`](ref:gateway/Seedcord) takes on its config. Every key in it is optional.
