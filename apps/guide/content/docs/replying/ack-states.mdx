...
description: seedcord checks your reply call before it reaches Discord, so the error you get names the method that works from there.
---
...
Its `cause` holds a second error whose stack points at the line that acknowledged the interaction first. This means that you'll always know exactly where that pesky double ack happened and what you need to use instead.
