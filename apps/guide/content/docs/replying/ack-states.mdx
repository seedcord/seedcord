...
[`send()`](ref:gateway/RepliableHandler.send) runs from every state, since it picks whichever verb the state allows. [Follow-ups and edits](/replying/more-messages) shows the mapping.
