...
When two handlers log the same message, the label tells you which one wrote it. A logger takes its label from the class holding it, so a line from `Ping` shows `Ping` as its label.
