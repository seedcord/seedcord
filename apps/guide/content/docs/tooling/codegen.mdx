---
title: Codegen
description: Type your commands, emojis, and plugins by running seedcord codegen, and catch a stale generated file in CI.
---

Where does TypeScript find the name of an option you declared? Let's take a look at an example.
...
Codegen records only the keys you attached. TypeScript resolves each plugin's type from the bot you built, since your `attach()` call already passes the plugin class. Your handler still reads `this.core.db` with every method that plugin declares.
...
A command constructor that throws fails the whole run, since codegen has to construct the class to read its options. `CliCodegenCommandConstructorThrew` shows the class, the file, and what your constructor threw. Keep anything that can fail out of a command constructor.
