---
title: Building
description: Compile your bot for production with seedcord build, from the steps it runs and starting the output to its config keys, build failures, and the runtime key.
---
...
It checks that your `entry` file exists, throwing `CliEntryNotFound` if it doesn't, then runs these steps in order.
...
Your CLI config holds these under `build`, and every one is optional.
...
A separate `tsconfig.build.json` lets you exclude tests and scripts from the build while your editor still type-checks them. Without `tsconfig`, seedcord looks for `tsconfig.build.json` first, then `tsconfig.json`. When neither exists, or the path you set doesn't, it throws `CliBuildTsconfigNotFound`.

## When the build fails

A type error stops the build. seedcord throws `CliBuildFailed` with the compiler's own output, shortened to its first 24,000 characters. The same code covers a project without `typescript` installed, since seedcord runs the `tsc` from your own dependencies.
...
You set `runtime` on your bot's own config in `src/bot.ts`, beside `bot` and `subscribers`. It says which kind of deployment your bot is built for, and leaving it out means `'server'`.
