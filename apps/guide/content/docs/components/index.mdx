---
title: Components
description: Anything your bot sends with structure gets a class over its discord.js builder, which gives it a name, types it, and carries your bot color.
---
...
Make that call in the constructor anyway. `this.instance` is protected to keep a ban card's whole description in one file. A call from your handler splits that description across two, so opening `BanCard.ts` tells you less than the whole story.
