---
title: Sources
description: Decide where a paginator's items come from, whether that's an array you already hold or one page at a time from your database.
---

A fifty-thousand-row mod log costs fifty thousand reads per click, to show ten rows.

What you pay per click comes down to the source. A source decides which items belong on page N. [Pagination](/components/pagination) used [`ArraySource`](ref:gateway/ArraySource), which loads the whole list on every click and slices it.
