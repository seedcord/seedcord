...
description: Page a message without a Paginator by calling paginate() with a cursor you declare, for when the class doesn't fit.
---
...
- **Extra values on the button.** A paginator's cursor carries a page and a slot. A filter, a sort key, or a target user id has to survive the page turn, so the nav button carries it, and shaping that id means writing your own [`CustomId`](ref:custom-id/CustomId).
