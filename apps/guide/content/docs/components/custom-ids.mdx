...
When the caller leaves the role out, pass `roleId: null` to `encode`. Each button then decodes `roleId` as `null`, which tells your handler the list has no filter.
