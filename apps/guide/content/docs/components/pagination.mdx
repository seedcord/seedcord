---
title: Pagination
description: Split a long list into pages with Paginator, and let users move between them with buttons that still work after a restart.
---
...
A [`Paginator`](ref:gateway/Paginator) sends one page of a list and routes the clicks on its nav buttons. Every button carries its target page inside the custom id, so a click still works after your bot restarts. It disables **Prev** on page one and **Next** on the last page.
