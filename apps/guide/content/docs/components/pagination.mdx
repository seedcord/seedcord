...
`prefix` sets the route every nav button carries, the same way a [`CustomId`](ref:custom-id/CustomId) prefix sets one. A click routes by its prefix alone, so pick a name nothing else in your bot uses.
