...
export function renderParameterBadge(parameter: FunctionSignatureModel['parameters'][number]): ReactElement {
    return (
        <span className="bg-surface-moderate text-subtle inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs">
            <span className="font-semibold text-(--text-function-mix-72)">{parameter.name}</span>
