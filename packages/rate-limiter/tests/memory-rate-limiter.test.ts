...
describe('MemoryRateLimiter', () => {
    ...
    it('reports not limited on the first charge of a fresh key', async () => {
        ...
        const result = await limiter.charge('user', { delay: 1000 });
    ...
    it('limits the second charge inside the window and reports when the slot frees', async () => {
        ...
        await limiter.charge('user', { delay: 1000 });
        const second = await limiter.charge('user', { delay: 1000 });

        expect(second.limited).toBe(true);
        expect(second.resetAt).toBe(Date.now() + 1000);
        expect(second.retryAfter).toBe(1000);
    ...
    it('frees the key again once the window passes', async () => {
        ...
        await limiter.charge('user', { delay: 1000 });
        const blocked = await limiter.charge('user', { delay: 1000 });
        expect(blocked.limited).toBe(true);

        vi.advanceTimersByTime(1000);

        const freed = await limiter.charge('user', { delay: 1000 });
    ...
    it('treats a limit below 1 as 1', async () => {
        ...
        const first = await limiter.charge('user', { delay: 1000, limit: 0 });
        const second = await limiter.charge('user', { delay: 1000, limit: 0 });
    ...
    it('evicts a fully expired key on the first access after the sweep gap, with no timer', async () => {
        ...
        await limiter.peek('other', { delay: 1000 });
    ...
    it('keeps a still-live key through the sweep', async () => {
        ...
        await limiter.charge('user', { delay: 120_000 });
    ...
    it('peek reports a fresh key as open without recording a use', async () => {
        ...
        const peeked = await limiter.peek('user', { delay: 1000, limit: 3 });

        expect(peeked).toMatchObject({ limited: false, remaining: 3, resetAt: 0, retryAfter: 0 });
        expect(limiter.size).toBe(0);
    });
    ...
    it('peek reports limited with the soonest reset but still records nothing', async () => {
        ...
        const peeked = await limiter.peek('user', { delay: 1000 });
        ...
        const peekedAgain = await limiter.peek('user', { delay: 1000 });
    ...
    it('reset clears the key so the next charge starts a fresh window', async () => {
        ...
        await limiter.charge('user', { delay: 1000 });
        const before = await limiter.peek('user', { delay: 1000 });
        expect(before.limited).toBe(true);

        await limiter.reset('user');

        expect(limiter.size).toBe(0);
        const after = await limiter.charge('user', { delay: 1000 });
        expect(after.limited).toBe(false);
    });

    it('brands resetAt as epoch ms', async () => {
        const result = await new MemoryRateLimiter().charge('user', { delay: 1000 });
