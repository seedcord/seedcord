...
describe('route-key parity between SlashTableGenerator and buildSlashRoute', () => {
    it('flat command route key equals buildSlashRoute(cmd)', () => {
        ...
        const tables: SlashTables = new SlashTableGenerator(silentLogger).generate([{ sourceFile: 'ban.ts', json }]);
    ...
    it('subcommand and group route keys equal buildSlashRoute over the same toJSON', () => {
        ...
        const tables: SlashTables = new SlashTableGenerator(silentLogger).generate([{ sourceFile: 'demo.ts', json }]);
    ...
    it('empty subcommand group yields no route on either side', () => {
        ...
        const tables: SlashTables = new SlashTableGenerator(silentLogger).generate([{ sourceFile: 'admin.ts', json }]);
