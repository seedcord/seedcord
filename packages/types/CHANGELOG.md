...
- `Config` gains `logger`, `store`, and `runtime`. ([`789f17a`](https://github.com/seedcord/seedcord/commit/789f17a))
    ...
    `Config` drops `shutdownEnabled` and `healthCheck`, which each transport config now declares. `clientOptions` and `events` move to `GatewayBotConfig` in `@seedcord/gateway`.
...
- `@seedcord/types` no longer depends on discord.js. `ReplyResponse` and its parts are structural types over `discord-api-types`. ([`789f17a`](https://github.com/seedcord/seedcord/commit/789f17a))
    ...
    `ReplyFile` is now `{ data: Uint8Array; name: string; description?; title? }`, which sends on either transport. the Gateway transport does still accept Djs' `AttachmentBuilder` though.
