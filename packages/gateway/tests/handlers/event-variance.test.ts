...
describe('event base variance', () => {
    it('rejects a single-event middleware where a multi-event one is expected', () => {
        ...
        expect(takesMultiMw).toBeTypeOf('function');
    });

    it('rejects a single-event handler where a multi-event one is expected', () => {
        ...
        expect(takesMultiHandler).toBeTypeOf('function');
    });
    ...
    it('rejects a multi-event middleware where a single-event one is expected', () => {
        ...
        expect(takesSingleMw).toBeTypeOf('function');
    });

    it('rejects a multi-event handler where a single-event one is expected', () => {
        // @ts-expect-error the union does not satisfy the single event name
        takesSingleHandler(multiHandler);

        expect(takesSingleHandler).toBeTypeOf('function');
    });

    it('accepts an exact match on both bases', () => {
