...
The rule fires only on a single fluent chain that carries both `setAutocomplete(true)` (a literal or a const flag whose type is the literal `true`) and `addChoices(...)` or `setChoices(...)` with at least one literal choice. Choices spread from a runtime array, and the two setters split across separate statements, are not flagged.
