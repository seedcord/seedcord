...
The rule reads both bounds from a single chain on any discord.js select menu builder (they all extend `BaseSelectMenuBuilder`). A bound is a numeric literal or a const whose type is a number literal, anything dynamic is not statically provable, so nothing is flagged. When a setter repeats, the last call is the one compared.
