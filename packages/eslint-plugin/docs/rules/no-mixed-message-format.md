...
The rule reads types, so it resolves each side wherever it comes from, an inline array, a variable, or a spread of another object, and a subclass of a v2 builder resolves through its base chain. An `ActionRowBuilder` is not a v2 builder, so it can sit alongside `content`. A v2 component reached through a seedcord `BuilderComponent`'s `.component` getter resolves the same way. A content field whose value is provably `undefined` does not count, JSON serialization drops it, so the wire payload is not mixed.
