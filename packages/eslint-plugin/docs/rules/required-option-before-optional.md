...
It checks only options whose required state is statically known, a literal boolean or a const flag whose type is the literal `true` or `false`. If any option in the chain reads `setRequired` from a plain boolean variable, or uses a callback the rule cannot read, the order is not statically provable and nothing is flagged.
