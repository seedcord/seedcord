...
// joins the arm names so a combinator gate's mismatch error names each arm
...
// message goes in a Constructor tuple, which TS prints inline where a raw context type would truncate. the
// labels are pre-computed strings, each transport supplies its own kind labeler and core never names a kind itself
